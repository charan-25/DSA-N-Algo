...
public class AddInterval {
    public static void main(String[] args) {
        int[][] arr = {{1,3},{6,9}};
        int[] interval = {2,5};
        int[][] res = addInterval(arr,interval);
        for(int i=0;i<res.length;i++){
            System.out.print(res[i][0]+" "+res[i][1]+",");
        }
    }

    public static int[][] addInterval(int[][] arr,int[] newInterval){
        List<int[]> res = new ArrayList<>();
        int index =0,n=arr.length;
        while(index<n && arr[index][1]<newInterval[0])
            res.add(arr[index++]);
        while(index<n && arr[index][0]<=newInterval[1]){
            newInterval[0] = Math.min(newInterval[0],arr[index][0]);
            newInterval[1] = Math.max(newInterval[1],arr[index][1]);
            index++;
        }
        res.add(newInterval);
        while(index<n){
